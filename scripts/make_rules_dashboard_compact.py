#!/usr/bin/env python3
"""results/charts/rules_dashboard_compact.png — 2x2 dashboard.

Four condition panels (Adversarial / Human-Generated Naturals /
LLM-Generated Naturals / Canonical). Per applier: no-rules scaffold + the
three diet cells (mean over the three rulebook draws), each pooled bar
flanked by thin semi-transparent strata bars — in-distribution (left, 5
tasks) and out-of-distribution (right, 7 tasks). Canonical cells from
orig_panel_cells.json (A38/A40 clean runs). Per-draw breakdown lives in the
appendix grid. No in-image title.
"""
import json
import pathlib
...
fig, axes = plt.subplots(2, 2, figsize=(13.8, 9.4), sharey=True)
axes = axes.ravel()
...
for ax, (cond, cname) in zip(axes, CONDS):
            ...
            if pool is not None:
                if iv is not None:
                    ...
                    ax.text(x - 0.29, iv + 0.7, f"{iv:.0f}", ha="center",
                            fontsize=6.2, color="#71808f", zorder=3)
                if oov is not None:
    ...
    ax.text(1.005, NO_REPH[cond], f"{NO_REPH[cond]:.1f}", fontsize=7.6,
            color=RED, ha="left", va="center", transform=ax.get_yaxis_transform())
    ax.set_xticks(ticks)
    ax.set_xticklabels(tlabels, fontsize=6.6, color="#4a5568")
    for gx, ap in zip(gticks, ["Claude", "Gemini", "Qwen"]):
        ax.text(gx, 4.6, ap, ha="center", fontsize=11, fontweight="bold",
                clip_on=False)
