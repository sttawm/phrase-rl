#!/usr/bin/env python3
"""A31 applies: one book x one applier x all three sealed conditions.
Usage: gen_a31_applies.py <book_path> <applier> <tag>   (FINAL_EVAL=1 gated)
Writes results/sealed/ph_a31_<tag>_<applier>_{adv,nat,orig}.parquet"""
import os
import sys
...
nat = pd.read_parquet(REPO / "results/sealed/ph_sealed_rephrase16.parquet")[["task", "k", "phrase"]]
...
for cond, bases in (("adv", adv), ("nat", nat), ("orig", orig)):
    ...
    out = REPO / f"results/sealed/ph_a31_{tag}_{applier}_{cond}.parquet"
