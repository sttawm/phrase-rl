...
NOW = 52.3  # ~04:20 Aug 7

ROWS = [
    ("L40S",    [("v10 RL (paused step ~256)", 4.0, 44.5, "train"),
                 ("stopped", 44.6, 47.5, "gen"),
                 ("v11 RL TRAINING — prompt B+, 25/50/25 mix, c4b  (9 min/step, runs all night)", 52.0, 76.0, "train")]),
    ("Eval 4",  [("v10 legs + 11 checkpoint cells", 39.0, 47.2, "rr"),
                 ("gen-stack install", 51.5, 52.2, "setup"),
                 ("v11 EVALUATOR — gen (B+) + roll on natural probe, stride 10", 52.3, 76.0, "worker")]),
    ...
    ("Eval 3",  [("v10 legs + cells", 39.0, 47.5, "rr"), ("STOPPED 04:05", 51.9, 53.3, "gen")]),
    ("Eval 6",  [("v10 gens + archive keeper + v11 naturals", 4.0, 51.6, "worker"),
                 ("STOPPED 04:05 (volume kept: single-copy v10 archives)", 51.9, 53.3, "gen")]),
...
ax.text(NOW + 0.2, len(ROWS) - 0.4, "now 04:20", color="#e53e3e", fontsize=8.5, fontweight="bold")
ax.set_yticks(range(len(ROWS)))
ax.set_yticklabels([p for p, _ in reversed(ROWS)], fontsize=9.5)
ticks = list(range(4, 77, 4))
ax.set_xticks(ticks)
ax.set_xticklabels([f"{t % 24:02d}:00" + ("\n(Aug 6)" if 24 <= t < 48 else ("\n(Aug 7)" if t >= 48 else "")) for t in ticks], fontsize=8)
ax.set_xlim(3.5, 76.5)
ax.grid(axis="x", alpha=0.25)
ax.legend(handles=[Patch(color=C[k], label=l) for k, l in
                   [("train", "RL training"), ("rr", "robustness leg (2,304 eps)"),
                    ("worker", "checkpoint evaluation"), ("setup", "setup"), ("gen", "stop")]],
          fontsize=8, loc="lower left")
ax.set_title("Steady state = 2 pods: L40S trains v11, Eval 4 evaluates its checkpoints. Eval 5 self-stops after the final ladder leg (~07:15); all others stopped.", fontsize=10)
fig.tight_layout()
