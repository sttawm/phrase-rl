...
NOW = 71.5  # 23:30 Aug 7 (hours from 00:00 Aug 5)
...
ROWS = [
    ...
    ("Eval 1",  [("(was Pod 5) qwen gens", 22.2, 39.9, "setup"), ("retired", 40.0, 41.4, "gen"),
                 ...
                 ("git gc: 24G archives + 47G .git", 70.6, 71.8, "setup"),
                 ("BANK SCORING shard 0 — 951 phrases, all night", 71.9, 80.0, "worker")]),
    ("Eval 6",  [("v10 gens + archive keeper", 4.0, 51.6, "worker"), ("stopped", 51.9, 53.0, "gen"),
                 ("v3 qwen leg (30.08)", 53.2, 61.0, "rr"),
                 ("v3 CLAUDE leg — layouts 12-23, last A28 leg", 63.4, 73.5, "rr"),
                 ("then BANK SCORING shard 1 (auto)", 73.6, 80.0, "worker")]),
    ("Eval 5",  [("v10 cells + legs", 39.0, 55.2, "rr"),
                 ("v3 qwen lay12 (30.82)", 57.3, 65.0, "rr"),
                 ("v3 CLAUDE leg — 36.02", 62.6, 71.0, "rr"),
                 ("auto-stop (nothing left)", 71.4, 72.6, "gen")]),
...
ax.text(NOW + 0.2, len(ROWS) - 0.4, "now 23:30", color="#e53e3e", fontsize=8.5, fontweight="bold")
ax.set_yticks(range(len(ROWS)))
ax.set_yticklabels([p for p, _ in reversed(ROWS)], fontsize=9.5)
ticks = list(range(4, 85, 4))
ax.set_xticks(ticks)
ax.set_xticklabels([f"{t % 24:02d}:00" + ("\n(Aug 6)" if 24 <= t < 48 else ("\n(Aug 7)" if 48 <= t < 72 else ("\n(Aug 8)" if t >= 72 else ""))) for t in ticks], fontsize=8)
ax.set_xlim(3.5, 84.5)
...
ax.set_title("A28 complete for qwen+gemini (v3 beats v4 everywhere). Overnight: v11 trains, e4 evaluates, "
             "e1+e6 score the 1,902-phrase bank for the rules loop. e5 auto-stops.", fontsize=9.5)
fig.tight_layout()
