#!/usr/bin/env bash
# v7d — reward-NOISE arm: REWARD_FRAMES 16 -> 32 (between-frame variance = 62% of
# reward-diff noise per the frames-vs-draws decomposition). beta=0.05 inherited.
...
mkdir -p "$IPC_DIR" results/checkpoints/phase2_v7d
...
tmux new-session -d -s score \
  "bash -lc 'set -o pipefail; eval \"\$(grep -E \"^export (HF_TOKEN|HF_HOME)\" ~/.bashrc || true)\"; export HF_HOME=\${HF_HOME:-/workspace/hf_cache}; cd /workspace/phrase-rl && \
   ...
   2>&1 | tee -a results/checkpoints/phase2_v7d/score_server.log; \
...
tmux new-session -d -s train \
  "bash -lc 'set -o pipefail; eval \"\$(grep -E \"^export (HF_TOKEN|HF_HOME|GEMINI_API_KEY)\" ~/.bashrc || true)\"; export HF_HOME=\${HF_HOME:-/workspace/hf_cache}; cd /workspace/phrase-rl && \
   .venv-gen/bin/python -m phrase_rl.phase2_train \
     ...
     --ckpt-dir results/checkpoints/phase2_v7d \
     --reward-mode verifier --reward-frames ${REWARD_FRAMES:-32} \
   ...
   2>&1 | tee -a results/checkpoints/phase2_v7d/train.log; \
