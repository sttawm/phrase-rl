#!/usr/bin/env python3
"""results/charts/swings_lollipop_full.png — dumbbell chart over ALL
significant swing sets (swing_sets_bridge.csv / swing_sets_libero.csv):
Bridge = every pair_verdicts survivor grouped into its designed set;
LIBERO = every significant pair grouped by (task, edit class)."""
import pathlib
...
panel(a1, B, "$\\pi_0$ on SIMPLER Bridge --- all %d significant sets (n = 72 per phrase)" % nb)
panel(a2, L, "$\\pi_{0.5}$ on LIBERO --- all %d significant sets (n = 50 per phrase)" % nl)
handles = [plt.Line2D([0], [0], marker="o", ls="", color=c, ms=6)
           for c in ["#4C72B0", "#DD8452", "#55A868", "#B8962E", "#8172B3", "#8C8C8C"]]
fig.legend(handles, ["Noun", "Color or modifier", "Verb",
                     "Preposition or particle", "Form, case, order", "Multi-edit"],
           fontsize=7.5, ncol=3, loc="lower center", frameon=False,
           bbox_to_anchor=(0.5, 0.0))
fig.tight_layout(rect=(0, 0.04, 1, 1))
out = R / "results/charts/swings_lollipop_full.png"
