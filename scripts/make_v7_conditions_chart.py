...
ax.plot([r["step"] for r in pol], [r["pooled"] for r in pol], marker="o", ms=8,
        color="#6b46c1", lw=2, label="v7 rewriting ORIGINALS (polish)")
if rep:
    ax.plot([r["step"] for r in rep], [r["pooled"] for r in rep], marker="s", ms=9,
            color="#dd6b20", lw=2, ls="-", label="v7 rewriting ADVERSARIAL (repair)")
ax.set_xlabel("v7 checkpoint step")
ax.set_ylabel("val-8 rollout success % (8 tasks × 24 layouts, greedy)")
ax.set_ylim(30, 58)
ax.set_title("v7 checkpoints on the full val-8 — both input conditions vs references")
ax.legend(loc="lower right", fontsize=9)
ax.grid(alpha=0.25)
fig.text(0.01, 0.01, "Repair points: ckpt 140 landed; 260 + late checkpoints (280-340) rolling. "
         "n=192/pt (±3.6). References: rolled val8 leg, n=288/task/arm.", fontsize=7.5, color="#4a5568")
