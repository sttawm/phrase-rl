...
b4_phrasing_rules_v4.md, using the SAME family wrap as the sealed ladder arms
...
FINAL_EVAL=1 required. Writes results/sealed/ph_sealed_rephrase16_rulesv4.parquet
...
for r in reph.itertuples():
    ...
    rows.append({"task": r.task, "arm": "rephrase16_rulesv4_gemini",
                 "phrase": out, "k": int(r.k), "src": r.phrase})
    print(f"PREFLIGHT [{r.task}] k={r.k:02d}: {r.phrase!r} -> {out!r}", flush=True)

pd.DataFrame(rows).to_parquet("results/sealed/ph_sealed_rephrase16_rulesv4.parquet", index=False)
print(f"ALL-REPHRASE-RULES-DONE n={len(rows)}")
