...
rephrase16 phrases, greedy -> results/sealed/ph_rr16_rulesv4_qwen.parquet.
...
out_path = "results/sealed/ph_rr16_rulesv4_qwen.parquet"
...
for r in reph.itertuples():
    ...
    rows.append({"task": r.task, "arm": "rr16_rulesv4_qwen", "phrase": p, "k": int(r.k), "src": str(r.phrase)})
    print(f"PREFLIGHT [{r.task}] k={r.k:02d}: {p!r}", flush=True)
pd.DataFrame(rows).to_parquet(out_path, index=False)
print(f"ALL-RULES-QWEN-DONE n={len(rows)}")
