...
\node[db] (evidence) at (12.45,0) {evidence};
...
\node[modelp] (llm) at (12.45,-2.3) {LLM};
\node[db] (rules) at (12.45,-4.7) {rules};
\draw[arr] (evidence) -- (llm);
\draw[arr] (llm) -- node[lab, right=1.2mm]{distill} (rules);

% ---------------- rule application (bottom, R -> L) ----------------
\node[modelg] (vlm2) at (9.9,-4.7) {VLM};
\node[card, align=center] (reph) at (4.7,-4.7) {%
  {\footnotesize\bfseries rephrased task instruction}\\[0.5mm]
  {\scriptsize``put the orange carrot on the yellow plate''}};
\node[model] (vla2) at (0.45,-4.7) {VLA};

\node[chip, fill=white, draw=cardEdge!35, font=\footnotesize\bfseries] (task2) at (9.9,-3.42) {task};
\draw[arr] (rules) -- (vlm2);
\draw[arr] (task2) -- (vlm2);
...
\begin{scope}[on background layer]
  ...
  \fill[bandB, fill opacity=0.38, rounded corners=4mm]
      (11.5,1.66) rectangle (13.55,-5.72);
  \fill[bandC, fill opacity=0.38, rounded corners=4mm]
      (-1.8,-2.95) rectangle (13.55,-5.72);
\end{scope}
...
\node[anchor=north west, font=\normalsize\bfseries, text=secC]
  at (-1.75,-5.86) {\textls[130]{RULE APPLICATION}};
