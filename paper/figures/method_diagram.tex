...
\draw[arr] (task) -- (vlm1);
\draw[arr] (vlm1.east) -- ($(adv.west)+(-0.05,-0.05)$);
\draw[arr] (vlm1.east) -- node[lab, above=0.6mm, pos=0.5]{generate}
                          node[lab, below=0.6mm, pos=0.5]{rephrases} (nat.west);
\draw[arr] (vlm1.east) -- ($(ben.west)+(-0.05,0.05)$);
\draw[arr] (adv.east) -- ($(vla1.west)+(-0.05,0.1)$);
\draw[arr] (nat.east) -- (vla1.west);
\draw[arr] (ben.east) -- ($(vla1.west)+(-0.05,-0.1)$);
\draw[arr] (vla1) -- (evidence);
\node[lab] at (11.52,-0.82) {execute \& score};
