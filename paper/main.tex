...
Much progress has been made towards building general purpose robots. These robots are capable of being instructed by humans via natural language to perform a wide variety of tasks. However, many of the state of the art, open robots, implemented as Vision-Language-Action models (VLAs), are still very sensitive to instruction phrasing: variation in task phrasing can change task success rates by more than 60 percentage points. We highlight this gap across multiple models and datasets. We then compare three different methods of learning a given VLA's preferred language. We package these learnings into a "rephraser" that sits between the human and the robot, translating each instruction into the VLA's preferred phrasing. Our best method raises VLA task success on adversarially phrased instructions from 26.6\% to 33.3\% (+6.7 points) and on naturally phrased instructions from 28.5\% to 33.9\% (+5.5 points), recovering 72\% of the success lost to natural rephrasing.
...
The obvious thing here is to characterize the success rate on
oracle, natural, and adversarial.

headroom is the highlight here. And our methods want to recover that.

we definitely should show some examples of oracle phrasings,
as well as adversarial and natural with numbers. 
...
We should rename v3 and v4 rules accordingly.

I'll want to highlight out of vocab improvements, despite in-vocab regression.

For our rules method, I'm not sure we need to go into great detail on our loop. Though we can. 
...
Seems that the LLM rules method mostly makes out of vocab better.

It's not entirely clear that our proxy is contributing much!
...
- adding an in-vocab clasifier might also be an easy way to improve performance since 
the rules methods tend to reduce 
- more simulation will lead to better weights-based RL comparison as well as a broader base from which to generalize rules; it's a little tricky to have to rely on a proxy
