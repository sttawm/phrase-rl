...
\begin{strip}
    \vspace{-1.2em}
    \centering
    \includegraphics[width=\textwidth]{training_flow_loop.png}
    \captionof{figure}{The phrase-search and rule-distillation loop. Each
    iteration, a VLM rephraser conditioned on the current rulebook and a
    per-input scene trace (\S\ref{sec:traces}) rewrites an evaluation set
    of original, natural, and adversarial instructions; the frozen
    executor's behavior on each rewrite is scored by simulated rollouts
    where the task is simulatable and by the trajectory proxy elsewhere.
    Scored rewrites accumulate in an evidence bank across iterations; an
    LLM judge audits whether the rewrites followed the rules, and the
    distiller reads the evidence and the audit alongside the previous
    rulebook to emit the next one.}
...
Our third method trains a VLM to map rephrased training tasks back to their original training task. In our evaluation, it helps with in-vocabulary tasks and hurts on out-of-vocabulary tasks. For future work, it may be interesting to endow this method with access to our proxy-reward function; on roughly a third of training tasks (69 of 213), our proxy-guided search found a phrasing that outscored the original.
...
only the ordering of phrases matters. We also train an ensemble of learned \emph{verifiers} over the
executor's full predicted-action features; alone it is equal to or worse
than the raw gripper signal at practical budgets
(Fig.~\ref{fig:reward_grids_gripper}). Where a calibrated
\emph{magnitude} is needed rather than a within-task ranking, we fit a
logistic success model on 254 phrases with rollout-measured success,
combining both channels; the fit weights the gripper channel roughly
$25\times$ the verifier ensemble.
...
the scene, written by an image-conditioned VLM from the robot's initial
camera frame. Traces are generated from the \emph{incoming} phrase,
...
distills the scored phrases into \emph{rollout-derived} (RD) rules. The
...
\S\ref{sec:reward}, with the RD rules informing candidate authoring; its
results are distilled, together with the RD rules and corpus statistics
(vocabulary counts over the training instructions), into the
\emph{rollout{+}training-data} (RTD) rules. The two turn out to occupy
...
\label{sec:grpo}
Our third method trains the rephraser against the reward directly. The
policy (Qwen3.5-9B with LoRA, trace-conditioned, text-only) samples
$K{=}16$ rephrases of each incoming instruction at temperature 1.0; each
candidate is scored by the frozen executor at ($F{=}4$, $C{=}5$) on the
gripper and verifier channels, blended as a within-group rank (weights
0.75/0.25); GRPO~\cite{shao2024deepseekmath} converts the group ranking
into signed, group-normalized advantages, under a KL leash
($\beta{=}0.15$) to the frozen base, without which the policy collapses.
Training inputs mix nominal, natural, and adversarial tiers (1:2:1), so
the policy must learn to repair rather than merely paraphrase. As
\S\ref{sec:results-comparison} reports, this recipe reliably optimizes
its reward without improving rollout success. A revised recipe --- the
calibrated success model of \S\ref{sec:reward} as reward in place of
within-group ranks (a magnitude cannot amplify scoring noise into
gradient the way a rank can), with duplicate-collapsed scoring funding
double the scoring contexts --- is specified and is the subject of
ongoing work. 
...
natural-rephrase arms roll all $K{=}16$ natural rephrases per task, one
episode per (rephrase, layout); conditions reported at uniform 24-layout
coverage total 4{,}608 episodes per arm, 12-layout halves 2{,}304.
Passthrough success differs across layout halves by up to 4.3 points
(executor-dependent) while treatment arms are flat across halves, so
half-grid numbers are used only for treatment comparisons.
...
rephrases per task, rewritten by each arm before reaching the executor.
Rollout-derived rules applied by Gemini restore pooled success to
36.9\%, level with the original-instruction reference (36.1); the same
frozen applier without rules reaches 31.2.}
...
appliers $\times$ \{RTD rules, RD rules, bare prompt\}. Identical rule
...
On \textbf{natural} inputs (Fig.~\ref{fig:natural_ladder}), RD rules
applied by Gemini restore pooled success to 36.9\% --- above the 36.1\%
...
RTD deepens repair to 33.3 (Gemini) and 34.3 (Claude; $+7.7$ over
passthrough, the deepest adversarial repair we measured) versus 31.6 for
RD. On \textbf{nominal} inputs the best arm does no harm: RTD applied by
Claude scores 37.7, slightly above the originals themselves. Gains are
OOV-concentrated throughout --- RTD on nominal inputs: OOV 30.9 vs 27.3
passthrough, while in-vocab gives back two points (46.0 vs 48.3) --- the
pattern that motivates the in-distribution gate discussed in
...
rollout-derived (RD) rules beat rollout{+}training-data (RTD) rules for
every applier --- $+1.0$ (Qwen), $+3.2$ (Gemini), $+5.2$ (Claude) ---
while RTD wins on adversarial inputs. The training-data evidence taught a
more conservative repair protocol that over-corrects benign variation.}
    \label{fig:a28}
\end{figure}

Fig.~\ref{fig:a28} isolates the rule-set difference: RD beats RTD on
naturals for every applier, and the ordering reverses under attack. The
training-data pass taught the RTD rules deeper repair of hostile inputs
at the price of over-correcting natural ones.
...
the capacity to comply. Frozen Qwen with the full rulebook scores 27.8 on
\emph{nominal} inputs, 8.3 points below leaving them untouched. Prompt
...
Every intervention pays a small in-vocab tax (rules: $-2.3$ on nominal
in-vocab tasks; SFT: its entire OOV loss) while earning its keep
...
RTD rules' advantage over RD is confined to adversarial inputs
($+1.7$ to $+3.3$); on naturals RD wins for every applier
