...
def build_prefix(args, instruction, image, trace):
    ...
    if fam == "bplusimg":
        return cover_prompt.build_single_phrase_prefix_bplusimg(instruction, image, trace=trace)
...
def save_latest(model, optimizer, state, args, ckpt_dir: Path):
    ...
    # dense checkpoint history for retro val evals) — snapshot cadence follows
    # --save-every (v8: 10; was hardcoded 20, which silently halved eval density)
    if state["step"] % max(getattr(args, "save_every", 20), 1) == 0:
        import shutil
...
def main():
    ...
    ap.add_argument("--prompt-family", choices=["cover", "bare", "bplus", "bplusimg"], default="cover",
