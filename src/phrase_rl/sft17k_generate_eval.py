        ...
        with torch.no_grad():
            if args.sample_k > 0:
                ...
                for si in range(args.sample_k):
                    ...
                    rows.append({"task": task, "arm": args.arm, "phrase": p,
                                 "instruction": instr, "sample_idx": si})
                print(f"[{args.arm}] {task}: {args.sample_k} samples, e.g. {rows[-1]['phrase']!r}")
            else:
                ...
                rows.append({"task": task, "arm": args.arm, "phrase": p, "instruction": instr})
                print(f"[{args.arm}] {task}: {instr!r} -> {p!r}")
