export const meta = {
  name: 'v2-distill-panels',
  description: 'Three one-shot rulebook distillations (train-only / sim-only / both), each a 3-lens panel with adversarial critique and synthesis',
  phases: [
...
const candidates = await parallel(ARMS.flatMap(arm =>
  LENSES.map(([lens, lensBrief]) => () =>
    agent(
      ...
      { label: `distill:${arm.key}:${lens}`, phase: 'Distill', schema: BOOK, effort: 'high' })
  )))

phase('Critique')
const byArm = {}
ARMS.forEach((arm, ai) => {
  byArm[arm.key] = candidates.slice(ai * 3, ai * 3 + 3).filter(Boolean)
})
const critiques = await parallel(ARMS.map(arm => () => {
  const books = byArm[arm.key].map((c, i) => `--- CANDIDATE ${i + 1} (${LENSES[i][0]} lens) ---\n${c.book}`).join('\n\n')
  ...
  return agent(
    ...
    { label: `critique:${arm.key}`, phase: 'Critique', schema: CRIT, effort: 'high' })
}))

phase('Synthesize')
const finals = await parallel(ARMS.map((arm, ai) => () => {
  const books = byArm[arm.key].map((c, i) => `--- CANDIDATE ${i + 1} (${LENSES[i][0]}) ---\n${c.book}`).join('\n\n')
  ...
  return agent(
    ...
    { label: `synthesize:${arm.key}`, phase: 'Synthesize', schema: BOOK, effort: 'xhigh' })
}))

return {
  train_only: finals[0] ? finals[0].book : null,
  sim_only: finals[1] ? finals[1].book : null,
  both: finals[2] ? finals[2].book : null,
  key_evidence: finals.map(f => f && f.key_evidence),
}