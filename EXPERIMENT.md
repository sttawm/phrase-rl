...
| Trainable phrase model | `Qwen/Qwen2.5-VL-7B-Instruct` | LoRA. Shared weights, two prompt modes (single-phrase, 16-list) |
...
- Timestep choice: sample a small number of t per episode, biased toward early frames (CoVer rephrases at boot time, so early-episode contexts matter most for rollout transfer).
...
2. **Advantage-weighted tuning:** generate 16 candidates per context (list prompt for diversity), score with frozen π0 + CRN, update under the single-phrase prompt:
   `L = -Σ_i max(A_i, 0) · log π_θ(y_i | c, p_single) + β·KL/SFT anchor + λ·list-format aux`
   Positive-only advantages first; signed advantages as ablation. This is advantage-weighted rephrase tuning, not exact GRPO.
...
- **0b — Sensitivity gate (HARD go/no-go):** ~200 val contexts × 16 Gemini rephrases, CRN scoring. Split the K draws in half → two loss estimates per phrase. **Go** iff phrase ranking reproduces across halves (high rank correlation) and phrase-to-phrase spread exceeds residual noise. Run both INTACT checkpoints (differential sensitivity is itself a finding). This is exactly the non-discriminative-reward failure mode from the earlier OpenVLA GRPO runs — catch it before training anything.
