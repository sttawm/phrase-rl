...
| Trainable phrase model | `Qwen/Qwen3-VL-8B-Instruct` | LoRA (~24GB w/ LoRA per Unsloth — fits GPU B). Shared weights, two prompt modes (single-phrase, 16-list). Supersedes Qwen2.5-VL-7B (older generation) |
...
- **0b — Sensitivity gate (HARD go/no-go):** ~200 val contexts × 32 Gemini rephrases (sensitivity phases use 32 for more rank points; training groups stay at 16), CRN scoring. Split the K draws in half → two loss estimates per phrase. **Go** iff phrase ranking reproduces across halves (high rank correlation) and phrase-to-phrase spread exceeds residual noise. Run both INTACT checkpoints (differential sensitivity is itself a finding). This is exactly the non-discriminative-reward failure mode from the earlier OpenVLA GRPO runs — catch it before training anything.
